...
| Python context API | `README.md`, `docs/ARCHITECTURE.md` | Context schema could be read as wrapped response or as shared schema superset | `modules/context-module/app.py`; `modules/shared/schemas.py` | Context app has local model and returns a bare param map | Documented actual `POST /analyze` request/response and noted shared/local schema duplication | High |
...
| Python fuzzer API | `README.md`, `docs/ARCHITECTURE.md` | Risk of documenting `/fuzz` or incomplete `/test` schema | `modules/fuzzer-module/app.py`; `modules/shared/schemas.py` | Implemented endpoint is `/test`; `/fuzz` is absent | Documented `/test`, `/health`, `/training/stats`; explicitly stated `/fuzz` does not exist | High |
| Health checks | `README.md`, `RUN.md`, `docs/ARCHITECTURE.md` | Health responses were incomplete | `core/src/health/*`, Python `app.py` files | Docs did not show actual response fields | Added Core aggregate health shape and Python service health fields | Medium |
| Docker/runtime artifacts | `README.md`, `RUN.md`, `docs/ARCHITECTURE.md`, `docs/REPOSITORY_GUIDE.md`, `docs/ML_GUIDE.md`, `dataset/README.md` | Mounted artifacts/fallbacks were incomplete | `docker-compose.yml`, Python app startup code | Missing model, ranker, dataset split, reports, and training data mount details | Documented all relevant mounts and fallback behavior | Medium |
...
| Dataset size | `README.md`, `docs/ARCHITECTURE.md`, `docs/ML_GUIDE.md`, `dataset/README.md` | Exact count `59,122` and historical `24K` style wording could be over-specific | Dataset docs/scripts not fully revalidated in this pass | Exact count should not be asserted without proof | Replaced/standardized canonical docs to “approximately 59K+” | Medium |
...
| Documentation hierarchy | `docs/README.md`, `docs/REPOSITORY_GUIDE.md` | Canonical docs and archive status needed stronger wording | Current docs layout | Historical files can conflict with current implementation | Added source-of-truth order and marked `docs/archive/` as historical/non-canonical | Medium |
...
A repository search was run for the requested stale/suspicious terms after documentation edits. Canonical docs were updated or qualified for:
