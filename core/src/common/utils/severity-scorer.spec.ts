...
describe('severity-scorer', () => {
  ...
  describe('getExecutionScore', () => {
    ...
    it('returns 2 for reflected only', () => {
      expect(
        getExecutionScore(makeInput({ reflected: true, executed: false })),
      ).toBe(2);
    });
    it('returns 1 for dom-only (neither reflected nor executed)', () => {
      expect(
        getExecutionScore(makeInput({ reflected: false, executed: false })),
      ).toBe(1);
    });
  });
  ...
  describe('getShareabilityScore', () => {
    ...
    it('returns 1 for URLSearchParams (client-side JS source)', () => {
      expect(
        getShareabilityScore(makeInput({ source: 'URLSearchParams' })),
      ).toBe(1);
    });
    it('returns 2 for postMessage', () => {
      expect(getShareabilityScore(makeInput({ source: 'postMessage' }))).toBe(
        2,
      );
    });
    ...
    it('returns 1 for location.hash', () => {
      expect(getShareabilityScore(makeInput({ source: 'location.hash' }))).toBe(
        1,
      );
    });
    it('returns 1 for document.cookie', () => {
      expect(
        getShareabilityScore(makeInput({ source: 'document.cookie' })),
      ).toBe(1);
    });
    it('returns 1 for unknown source (conservative)', () => {
      expect(getShareabilityScore(makeInput({ source: 'window.name' }))).toBe(
        1,
      );
    });
  });
  ...
  describe('getSinkDangerScore', () => {
    ...
    it('returns 3 for location_assign', () => {
      expect(getSinkDangerScore(makeInput({ sink: 'location_assign' }))).toBe(
        3,
      );
    });
  ...
  describe('getPayloadScore', () => {
    it('returns 3 for document.cookie in payload', () => {
      expect(
        getPayloadScore(makeInput({ payload: 'x(document.cookie)' })),
      ).toBe(3);
    });
    it('returns 2 for localStorage in payload', () => {
      expect(
        getPayloadScore(makeInput({ payload: 'localStorage.getItem("x")' })),
      ).toBe(2);
    });
    ...
    it('returns 1 for browserAlertTriggered', () => {
      expect(
        getPayloadScore(
          makeInput({ payload: 'x', browserAlertTriggered: true }),
        ),
      ).toBe(1);
    });
    it('stacks document.cookie + %', () => {
      expect(
        getPayloadScore(makeInput({ payload: '%27(document.cookie)' })),
      ).toBe(4);
    });
    it('returns 0 for clean payload', () => {
      expect(
        getPayloadScore(makeInput({ payload: '<img src=x onerror=alert(1)>' })),
      ).toBe(0);
    });
  ...
  describe('real findings regression', () => {
    ...
    it('Finding 9: /comment reflected+url_param+attribute+cookie+% → CRITICAL', () => {
      const r = scoreFinding({
        ...
        sink: 'attribute',
        payload:
          '<!--><svg+onmouseover=%27top[%2fal%2f...](document.cookie)%27>',
        exactMatch: true,
        browserAlertTriggered: false,
      });
      expect(r.severity).toBe(VulnSeverity.CRITICAL);
